...
import { Chat, detaDB, Integration, Prompt, Settings } from "../db";
...
import { ChatContext, ChatsContext, IncognitoModeContext, IntegrationsContext, PromptsContext, SettingsContext } from "../hooks/contexts";
...
export function BaseLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const dataFetch = async () => {
      const { items } = await detaDB.integrations.fetch({});
      if (items) {
        setIntegrations(items as unknown as Integration[]);
      }
  ...
  return (
    <SettingsContext.Provider value={{ settings: settings, setSettings: setSettings }}>
      <IntegrationsContext.Provider value={{ integrations: integrations, setIntegrations: setIntegrations }}>
